...
class SqliteResultParser extends ResultParser {
  ...
  List<Map<String, dynamic>> parse(List<Map<String, dynamic>> result) {
    ...
    for (var r in result) {
      ...
      final relationsToParse = [
        for (var entry in currentRow.entries)
          table.relations
              .where((element) => element.column == entry.key)
              .firstOrNull
      ].where((element) => element != null).toList();
      for (final relation in relationsToParse) {
        final relationEntity = relations
        ...
        for (var entry in currentRow.entries) {
          if (relationTable.columns
              .where((element) =>
                  element.name == entry.key ||
                  entry.key.split('.').last == element.name)
              .isNotEmpty) {
            entries.add(entry);
          }
        }
  ...
  dynamic _parseRelation(
    ...
    final columns = Map<String, dynamic>.fromEntries(
        relationColumns.map((e) => MapEntry(e.key.split('.').last, e.value)));
    return columns;
  }
