...
class SqlOperatorTransformer extends Transformer{
  ...
  String transform(covariant Operator element) {
    return switch(element.runtimeType.toString()){
      "In" => ' IN (${element.value.map((e) => e is String ? '"$e"' : e).join(', ')})',
      "NotIn" => ' NOT IN (${element.value.map((e) => e is String ? '"$e"' : e).join(', ')})',
      "Equal" => ' = ${element.value is String ? '"${element.value}"' : element.value}',
      "NotEqual" => ' != ${element.value is String ? '"${element.value}"' : element.value}',
      "And" => ' AND',
      "Or" => ' OR',
